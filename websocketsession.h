...
public:
    explicit WebsocketSession ( net::io_context& ioc, ssl::context& ctx, std::string const &host, int port);
    ~WebsocketSession();

    void connect ();
    void send(std::shared_ptr<const std::string> msg, std::weak_ptr<UnixDomainSession> uds);
